...
-- Assign user paramters as script variables.
local spacing = tonumber(g.getstring("Enter the spacing of the configurations (center to center distance):", "100"))
local shape_live = g.getstring("Would you like a circle or ellipse for the live sites (enter C or E):", "E"):upper()
local dim_live = g.getstring("Enter the dimensions for the live circle/ellipse (e.g., radius or major,minor):", "25,20")
local range_setback = g.getstring("Enter the range of setbacks as min,max (e.g., 5,15):", "5,15")
local shape_dead = g.getstring("Would you like a circle or ellipse for the dead sites (enter C or E):", "C"):upper()
...
if shape_dead == "C" then
    range_radius_dead = g.getstring("Enter the range of radii for the dead circle as min,max (e.g., 5,10):", "5,10")
elseif shape_dead == "E" then
    range_axes_dead = g.getstring("Enter the range of major/minor axes for the dead ellipse, major_range,minor_range (e.g., 5,20 5,20):", "5,20 5,20")
else
    g.warn("Invalid shape input. Please enter 'C' for circle or 'E' for ellipse.")
end
local configs_per_row = tonumber(g.getstring("Enter the number of configurations per row:", "20"))
-----------------------------------------------------------------------------------------------
...
-- Format of CSV file
file:write("Shape of Live Cells,Radius or Axis Lengths of Live Shape,Y Setback,Shape of Dead Cells,Radius or Axis Lengths of Dead Shape\n")
-----------------------------------------------------------------------------------------------
        ...
        for major = (axes_dead_min[1] or 0), (axes_dead_max[1] or 0) do -- Iterate through major axes range.
            for minor = (axes_dead_min[2] or 0), (axes_dead_max[2] or 0) do -- Iterate through minor axes range.
                ...
                if shape_live == "C" then -- Live shape is a circle.
                    ...
                    drawFilledEllipse(current_x, current_y, tonumber(live_dims[1]), tonumber(live_dims[2]), 1)
                end

                if shape_dead == 'C' then
                    dead_shape_size = tostring(radius_dead)
                    drawFilledCircle(current_x, current_y + setback, radius_dead, 0)
                else -- Dead shape is an ellipse
