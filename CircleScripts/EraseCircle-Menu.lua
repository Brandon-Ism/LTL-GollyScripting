...
local function drawFilledCircle(cx, cy, r)
    -- Function to fill cells (dead) in quadrants 1-4
    local function fillQuadrants(x, y)
        for dy = 0, y do -- vertical
            g.setcell(cx + x, cy + dy, 0)
            g.setcell(cx - x, cy + dy, 0)
            g.setcell(cx + x, cy - dy, 0)
            g.setcell(cx - x, cy - dy, 0)
        end
        for dx = 0, x do -- horizontal
            g.setcell(cx + dx, cy + y, 0)
            g.setcell(cx - dx, cy + y, 0)
            g.setcell(cx + dx, cy - y, 0)
            g.setcell(cx - dx, cy - y, 0)
        end
...
-- Function to draw ring (dead) given center (x,y) and radius using Bresenham's circle drawing algorithm
...
-- Function to show user cell preview of circle as cursor is moved across grid to radius length.
    ...
    while true do
        ...
        if event:find("click") == 1 then
            ...
            if started then 
                ...
                oldCells = {}  -- Clear the old cells after finalizing a circle
...
while true do
    local choice = g.getstring("Choose a method to create a circle (dead cells):\n"..
                               "1) Draw filled circle with a cell preview.\n"..
                               "2) Input filled circle dimensions.\n"..
                               "3) Draw ring with a cell preview.\n"..
                               "4) Input ring dimensions.", "1")
    ...
    elseif choice == "2" or choice == "4" then
        -- Input dimensions to draw filled or unfilled circles
        local center = g.getstring("Enter center as x,y:", "0,0")
        local radius = g.getstring("Enter radius:", "10")
